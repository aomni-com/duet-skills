...
import type { Skill, SkillMetadata, UseCase, UseCaseCategory } from './types.js'
...
const DUET_NODE = { label: 'Duet', icon: 'sparkle', tone: 'duet' as const }
...
export const USE_CASES: readonly UseCase[] = [
  ...
  },
  {
    ...
    seedPrompt:
      'Build a usage-based customer health monitor. Pull live usage signals, score each account, and alert when one flips at-risk. Draft proactive outreach for at-risk accounts and post a weekly health board. Start by asking: where usage data lives, which signals predict churn for my product, and who owns outreach when an account turns red.',
  ...
  /* ──────────────────────────── Operations ─────────────────────────── */
  {
    ...
    seedPrompt:
      'Build an end-to-end inbox flow: triage every inbound email into a clear bucket, draft replies for the ones that need one, and queue them for my approval — never auto-send. Send me a weekly recap. Start by asking: which labels / filters define the inbox to manage, the tone + signature to use, and any topics I never want auto-drafted.',
    primarySkillId: 'email-triage',
    supportingSkillIds: ['composio-credentials'],
    surfaces: ['home', 'desktop'],
  },
  ...
  /* ──────────────────────────── Personal assistant ─────────────────────────── */
  {
    ...
    seedPrompt:
      'Run my calendar. Each morning post a daily plan with prep notes per meeting, propose reschedules when conflicts appear, and protect buffer + focus blocks automatically. Start by asking: my working hours, focus blocks to protect, prep note length, and whether reschedules should auto-apply or only propose.',
  ...
  },
  {
    ...
    seedPrompt:
      'Set up reminders + scheduled check-ins. Send context-aware reminders (not just clock-time), run recurring check-ins on goals / habits / projects, allow easy snooze + cadence change, and post a weekly review. Start by asking: what to remind me about, when I want to be interrupted vs deferred, and whether check-ins should be light or push me.',
    primarySkillId: 'cron',
...
export function getUseCaseById(id: string | null | undefined): UseCase | undefined {
...
export function getAllSkillIdsForUseCase(useCase: UseCase): readonly string[] {
 ...
 */
export function getOrderedUseCases(preferredCategory: UseCaseCategory): readonly UseCase[] {
  const order: UseCaseCategory[] = [
    preferredCategory,
    ...(Object.keys(CATEGORY_LABELS) as UseCaseCategory[]).filter((c) => c !== preferredCategory),
  ]
 ...
 */
const ROLE_KEYWORD_PATTERNS: Array<{ pattern: RegExp; category: UseCaseCategory }> = [
  {
    pattern:
      /\b(eng|engineer|developer|dev|software|backend|frontend|platform|infra|sre|devops)\b/i,
    category: 'engineering',
  },
  {
...
export function resolveUseCaseCategoryFromRole(
  signal: string | undefined | null,
): UseCaseCategory {
  if (!signal) return 'internal-tools'
